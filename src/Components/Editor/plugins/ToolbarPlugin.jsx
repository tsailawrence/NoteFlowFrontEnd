...
import { INSERT_YOUTUBE_COMMAND } from "../plugins/YoutubePlugins/YoutubePlugin";
...
import {
  ...
  YouTubeNode,
  $isYouTubeNode,
} from "../plugins/YoutubePlugins/YouTubeNode";
...
export default function ToolbarPlugin() {
  ...
  function useConsecutiveKeys(callback) {
    const [prevKeys, setPrevKeys] = useState([]);

    function handleKeyDown(event) {
      const newKeys = [...prevKeys, event.key];
      setPrevKeys(newKeys.slice(-3));
      if (
        newKeys[newKeys.length - 3] === "/" &&
        newKeys[newKeys.length - 2] === "h"
      ) {
        event.preventDefault();
        callback(newKeys[newKeys.length - 1]);
      }
    }
